	...
	$(document).keyup( e=> {
		if (e.keyCode == 17) {
		  if (e.originalEvent.location == 1)
			  dc.send('CTRL key is pressed.');
		  else
			  dc.send('CTRL key is pressed.');
	  } else if (e.keyCode == 18) {
		  if (e.originalEvent.location == 1)
				dc.send('ALT key is pressed.');
		  else
			  dc.send('ALT key is pressed.');
...
conn.onerror = error =>{
	swal({title: "Oops ! Network Error",text: "Automatically Re-connecting ...",icon: "error",buttons: false,timer: 2000});
	setTimeout(()=>{location.reload();}, 2500);
}
conn.onclose = reason =>{
	swal({title: 'The Server is busy Now',text: 'Automatically Reconnecting ...',icon: "info",timer: 2500,buttons: false});
	setTimeout(()=>{location.reload();}, 2500);
}
conn.onopen = () =>{
	giveCamera
	...
	},error=>{swal({title: "Screen Capture Permission Denied",text: "Reload the Page & Grant Permissions",icon: "error",buttons: false});})
	},error=>{swal({title: "Camera Permission Denied",text: "Grant Permissions & Reload the Page",icon: "error",buttons: false});})	
}

conn.onmessage = msg =>{
	payload = JSON.parse(msg.data);
	if(payload.candidate){
		webrtconn.addIceCandidate(new RTCIceCandidate(payload.candidate)).catch(e =>{});
	}
	if(payload.sdp){ 
		webrtconn.setRemoteDescription(payload.sdp);
	}
	if(payload.type=="MAKEOFFER"){
		webrtconn.createOffer()
		.then(offer=>{return webrtconn.setLocalDescription(offer)})
		.then(()=>{
			toServer({
					from:"CLIENT",
					to:"PROCTOR",
